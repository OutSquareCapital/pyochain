...
- **API change**: `PyoIterable::{all, any, join, sum, min, my_by, max, max_by}` are moved to `PyoIterator`. Simply add a call to `iter()` in impacted code if a `PyoCollection` was used.
