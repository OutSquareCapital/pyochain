...
>: PyWrapper + PyTypeInfo
{
    ...
    fn cmp_eq(&self, right: Bound<'py, PyAny>) -> PyCmpOut<'py, bool> {
        ...
        try_cast! {
            match right {
                CaseExact::Set(set) | CaseExact::SetMut(set) => inner.eq(set.get().bind(py)).map(Either::Left),
    ...
    fn cmp_and(&self, other: SetCmp<'py>) -> PyResult<Bound<'py, T>> {
        match &other {
            SetCmp::Set(x) => self
                .bind(x.py())
                .bitand(x.get().bind(x.py()))
                .map(|x| unsafe { x.cast_into_unchecked::<T>() }),
            SetCmp::SetMut(x) => self
                .bind(x.py())
                .bitand(x.get().bind(x.py()))
                .map(|x| unsafe { x.cast_into_unchecked::<T>() }),
            SetCmp::PySet(x) => self
                ...
                .bitand(x)
                .map(|x| unsafe { x.cast_into_unchecked::<T>() }),
            SetCmp::PyFrozen(x) => self
                .bind(x.py())
                .bitand(x)
                .map(|x| unsafe { x.cast_into_unchecked::<T>() }),
            SetCmp::PyAbstract(x) => Self::handle_pyabstract_set(x.bitand(&**self)?),
        }
    }
    ...
    fn cmp_or(&self, other: SetCmp<'py>) -> PyResult<Bound<'py, T>> {
        match &other {
            ...
            SetCmp::PyFrozen(x) => self
                ...
                .bitor(x)
                .map(|x| unsafe { x.cast_into_unchecked::<T>() }),
            SetCmp::PyAbstract(x) => Self::handle_pyabstract_set(x.bitor(&**self)?),
        }
    }
    ...
    fn cmp_xor(&self, other: SetCmp<'py>) -> PyResult<Bound<'py, T>> {
        match &other {
            ...
            SetCmp::PyFrozen(x) => self
                ...
                .bitxor(x)
                .map(|x| unsafe { x.cast_into_unchecked::<T>() }),
            SetCmp::PyAbstract(x) => Self::handle_pyabstract_set(x.bitxor(&**self)?),
        }
    }
    ...
    fn cmp_sub(&self, other: SetCmp<'py>) -> PyResult<Bound<'py, T>> {
        match &other {
            ...
            SetCmp::PyFrozen(x) => self
                ...
                .sub(x)
                .map(|x| unsafe { x.cast_into_unchecked::<T>() }),
            SetCmp::PyAbstract(x) => Self::handle_pyabstract_set(self.bind(x.py()).sub(x)?),
        }
    }
}
...
impl SetMut {
    ...
    fn __iand__(&self, value: Bound<'_, PyAny>) -> PyResult<()> {
        self.bind(value.py()).intersection_update((value,))
    }
    fn __ior__(&self, value: Bound<'_, PyAny>) -> PyResult<()> {
        self.bind(value.py()).update((value,))
    }
    fn __isub__(&self, value: Bound<'_, PyAny>) -> PyResult<()> {
        self.bind(value.py()).difference_update((value,))
    }
    fn __ixor__(&self, value: Bound<'_, PyAny>) -> PyResult<()> {
        self.bind(value.py()).symmetric_difference_update(value)
    }
    ...
    fn discard(&self, value: Bound<'_, PyAny>) -> PyResult<()> {
        self.bind(value.py()).discard(value)?;
        Ok(())
    }
    #[pyo3(signature = (*s))]
    fn intersection_update(&self, s: Bound<'_, PyTuple>) -> PyResult<()> {
        self.bind(s.py()).intersection_update(s)
    }
    fn isdisjoint<'py>(&self, s: Bound<'py, PyAny>) -> PyResult<Bound<'py, PyBool>> {
        self.bind(s.py()).isdisjoint(s)
    }
    ...
    fn remove(&self, element: &Bound<'_, PyAny>) -> PyResult<()> {
        self.bind(element.py()).remove(element)
    }

    fn symmetric_difference_update(&self, s: Bound<'_, PyAny>) -> PyResult<()> {
        self.bind(s.py()).symmetric_difference_update(s)
    }
    #[pyo3(signature = (*others))]
    fn intersection<'py>(&self, others: Bound<'py, PyTuple>) -> PyResult<Bound<'py, Self>> {
        self.bind(others.py())
    ...
    fn difference_update(&self, s: Bound<'_, PyTuple>) -> PyResult<()> {
        self.bind(s.py()).difference_update(s)
    }
    fn symmetric_difference<'py>(&self, other: Bound<'py, PyAny>) -> PyResult<Bound<'py, Self>> {
        self.bind(other.py())
            .symmetric_difference(other)
